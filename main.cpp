...
void HeuConstAleGul(DadosDasTarefas infoTarefas[], const int percentual, int MAXmakespan){
   ...
   for(int j = 0; j < (QuantidadeDeTarefas+2); j++){
    ...
    // memcpy(recursos, tempERecursos, sizeof(tempERecursos)*MAX_TAREFAS);
...
void CalculoFo(DadosDasTarefas infoTarefas[]){
    ...
    //Penalizacao por violacao de precedencia
    /*for(int i=0; i<(QuantidadeDeTarefas+2); i++){
        if(infoTarefas[i].TempoDeInicio<= infoTarefas[infoTarefas[i].IdDoAntecessor].TempoDeInicio + infoTarefas[infoTarefas[i].IdDoAntecessor].DuracaoDaTarefa){
            vio_pre+=infoTarefas[i].DuracaoDaTarefa;
        }
    }*/

    FO += (10*vio_pre);

    //Penalizacao por violacao de Recursos
  /*  for(int i=0; i<MaiorMKSPpossivel; i++){
        for(int j=0; j<(QuantidadeDeRecursos); j++){
           /* if(tempoerecursos[i].RecursosUsadosNoTempo[j]<0){
                vio_rec+=QuantidadeDeCadaRecursoDisponivel[j];
            }
        }
    }*/
